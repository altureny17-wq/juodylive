...
class _PrebuildAudioRoomScreenState extends State<PrebuildAudioRoomScreen> with TickerProviderStateMixin {
  ...
  // ✅ دالة مستقلة لبناء الـ config - تضمن تطبيق جميع الخصائص على host و audience
  ZegoUIKitPrebuiltLiveAudioRoomConfig _buildRoomConfig(
    ...
    config.seat.layout.rowConfigs = List.generate(numberOfSeats, (index) {
      if (index == 0) {
      ...
      if (widget.liveStreaming!.getNumberOfChairs == 20) {
        return ZegoLiveAudioRoomLayoutRowConfig(
            count: 5, alignment: ZegoLiveAudioRoomLayoutAlignment.start);
      }
      if (widget.liveStreaming!.getNumberOfChairs == 24) {
        return ZegoLiveAudioRoomLayoutRowConfig(
            count: 6, alignment: ZegoLiveAudioRoomLayoutAlignment.start);
      }
      return ZegoLiveAudioRoomLayoutRowConfig(
          count: 4,
              ...
              onGiftSelected: (gift) {
                ...
                QuickHelp.showAppNotificationAdvanced(
                  ...
                  message: "live_streaming.gift_sent_explain".tr(
                    namedArgs: {"name": widget.liveStreaming!.getAuthor!.getFirstName!},
                  ),
                  isError: false,
                );
              },
  ...
  sendGift(GiftsModel giftsModel, UserModel mUser) async {
    ...
    QuickHelp.saveReceivedGifts(receiver: mUser, author: widget.currentUser!, gift: giftsModel);
    QuickHelp.saveCoinTransaction(
      receiver: mUser,
      author: widget.currentUser!,
      amountTransacted: giftsModel.getCoins!,
    );
    QueryBuilder<LeadersModel> queryBuilder = QueryBuilder<LeadersModel>(LeadersModel());
    queryBuilder.whereEqualTo(LeadersModel.keyAuthorId, widget.currentUser!.objectId!);
    ...
    if (parseResponse.success) {
      ...
      if (parseResponse.results != null) {
        ...
        leadersModel.incrementDiamondsQuantity = giftsSentModel.getDiamondsQuantity!;
      ...
      if(mUser.objectId == widget.liveStreaming!.getAuthorId) {
        widget.liveStreaming!.addDiamonds = QuickHelp.getDiamondsForReceiver(giftsModel.getCoins!);
        await widget.liveStreaming!.save();
      ...
      children: [
                        ...
                        child: Row(
                          ...
                          children: [
                              ...
                              children: [
                                ...
                                SizedBox(
                                  ...
                                  child: TextWithTap(
                                    ...
                                    marginTop: 2, overflow: TextOverflow.ellipsis,
                                    alignment: Alignment.center,
                                  ),
                                ),
                              ],
                            ...
                            TextWithTap(
                              ...
                              marginRight: 5, marginLeft: 5, textItalic: true,
                            ),
                            Column(
                              mainAxisSize: MainAxisSize.min,
                              children: [
                      ...
                      children: [
                        Column(
                          ...
                          children: [
                            ...
                            ContainerCorner(
                              color: kTransparentColor, marginTop: 1,
                              child: Row(
                                mainAxisAlignment: MainAxisAlignment.center,
                                children: [
                                  SvgPicture.asset("assets/svg/ic_coin_with_star.svg", width: 10, height: 10),
                                  TextWithTap(
                                    showGiftSendersController.receivedGiftList[index].getCoins.toString(),
                                    color: Colors.white, fontSize: 10,
                                    marginLeft: 5, fontWeight: FontWeight.w900,
                                  )
                                ],
                              ),
                            ),
                        ...
                        TextWithTap(
                          "x1", color: Colors.white,
                          fontWeight: FontWeight.w900, fontSize: 25,
                          marginLeft: 10, textItalic: true,
                        ),
                      ],
                    )
                  ],
        ...
        Obx((){
          return Visibility(
            ...
            child: advanceMediaPlayer(playerSize: const Size(120, 160)),
          );
        }),
      ],
    );
  ...
  toggleSharingMedia() async{
    ...
    if(response.success && response.results != null) {
      ...
      QuickHelp.hideLoadingDialog(context);
      QuickHelp.showAppNotificationAdvanced(
        title: "error".tr(), message: "not_connected".tr(),
  ...
  Widget svgaWidget(GiftsModel giftItem) {
    int level = 1;
    if (giftItem.getCoins! < 10) { level = 1; }
    else if (giftItem.getCoins! < 100) { level = 2; }
    else { level = 3; }
    switch (level) {
      case 2:
        return Positioned(
          top: 100, bottom: 100, left: 1, right: 1,
          child: ZegoSvgaPlayerWidget(
            key: UniqueKey(), giftItem: giftItem,
            onPlayEnd: () { ZegoGiftManager().playList.next(); }, count: 1,
          ),
        );
      case 3:
    ...
    return Positioned(
      bottom: 200,
      child: ZegoSvgaPlayerWidget(
        key: UniqueKey(), size: const Size(100, 100), giftItem: giftItem,
        onPlayEnd: () { ZegoGiftManager().playList.next(); }, count: 1,
      ),
    );
  }
  ...
  setupLiveGifts() async {
    QueryBuilder<GiftsSentModel> queryBuilder = QueryBuilder<GiftsSentModel>(GiftsSentModel());
    queryBuilder.whereEqualTo(GiftsSentModel.keyLiveId, widget.liveStreaming!.objectId);
  ...
  void openUserToReceiveCoins() async {
    showModalBottomSheet(
      ...
      enableDrag: true,
      isDismissible: true,
      builder: (context) { return _showUserToReceiveCoins(); },
    );
  }
  ...
  Widget _showUserToReceiveCoins() {
    ...
    return ContainerCorner(
      ...
      width: size.width, borderColor: Colors.white,
      radiusTopLeft: 10, radiusTopRight: 10,
      marginRight: 15, marginLeft: 15,
      child: Column(
        children: [
          TextWithTap(
            "choose_gift_receiver".tr(), color: Colors.white,
          ...
          Flexible(child: ParseLiveGridWidget<UserModel>(
            query: coHostQuery, crossAxisCount: 4,
            reverse: false, crossAxisSpacing: 5, mainAxisSpacing: 10,
            lazyLoading: false, padding: EdgeInsets.only(left: 15, right: 15),
            childAspectRatio: 0.7, shrinkWrap: true,
            listenOnAllSubItems: true, duration: Duration(seconds: 0),
            animationController: _animationController,
            childBuilder: (BuildContext context, ParseLiveListElementSnapshot<UserModel> snapshot) {
              ...
              return GestureDetector(
                ...
                child: Column(
                  ...
                  children: [
                    QuickActions.avatarWidget(user, width: size.width / 5.5, height: size.width / 5.5),
                    TextWithTap(user.getFullName!, color: Colors.white, marginTop: 5,
